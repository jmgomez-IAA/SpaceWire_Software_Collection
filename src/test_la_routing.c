...
int __cdecl  main(int argc, char * argv[]){
  ...
  uint32_t offset = 0;
  uint32_t dev_address = 0x45;
  //  uint32_t val_data;
  U8 val_data[] = {0x00, 0x00, 0x00, 0x80};
  U8 val_data2[] = {0x00, 0x00, 0x00, 0x0D};
  for(opCounter = 0 ; opCounter < (rtr_config_cmd-2); ++ opCounter){
    offset =  (dev_address+opCounter)*reg_byteSize;
  ...
  // val_data = opCounter;
  status = LoopBackPacketToStream ( (vTxStreamItem + port_config_cmd)+opCounter , 0x45, 0xFE, val_data, reg_address);
  //val_data = opCounter + port_config_cmd;
  status = LoopBackPacketToStream ( (vTxStreamItem + port_config_cmd)+opCounter+1 , 0x46, 0xFE, val_data, reg_address);
